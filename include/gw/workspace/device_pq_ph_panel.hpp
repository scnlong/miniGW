...
// ordinary C++ translation units without including CUDA headers.
class DeviceComplexPanelView {
public:
    DeviceComplexPanelView() = default;
...
private:
    friend class DevicePqPhPanelView;

    DeviceComplexPanelView(void* data, std::size_t rows, std::size_t cols) noexcept
...
enum class DevicePqPhPanelStorageMode {
    Auto,
    FullEriResident,
    StreamingPanel
};

[[nodiscard]] std::string_view to_string(DevicePqPhPanelStorageMode mode) noexcept;

// CUDA-resident/streaming panel source for P_{p k, i a} = (p k | i a).
//
// Two modes are supported:
//
//   FullEriResident:
//     Upload the full replicated ERI tensor to one GPU and assemble each panel
//     with a CUDA kernel.  This avoids per-panel H2D traffic but can exhaust
...
// implementation.
class DevicePqPhPanelView {
public:
    DevicePqPhPanelView(const MolecularIntegrals& integrals,
                        const ParticleHoleBasis& ph_basis,
                        std::size_t max_panel_width,
                        DevicePqPhPanelStorageMode mode = DevicePqPhPanelStorageMode::Auto);
    ~DevicePqPhPanelView();

    DevicePqPhPanelView(const DevicePqPhPanelView&) = delete;
    DevicePqPhPanelView& operator=(const DevicePqPhPanelView&) = delete;

    DevicePqPhPanelView(DevicePqPhPanelView&&) noexcept;
    DevicePqPhPanelView& operator=(DevicePqPhPanelView&&) noexcept;
    ...
    [[nodiscard]] DevicePqPhPanelStorageMode storage_mode() const noexcept;
    ...
    [[nodiscard]] DeviceComplexPanelView fill_panel(std::size_t p_index,
                                                    std::size_t k_begin,
                                                    std::size_t width);
