...
BackendCapabilities CosmaPxgemmBackend::capabilities() const noexcept {
    BackendCapabilities caps = scalapack_.capabilities();
...
MatrixComplex CosmaPxgemmBackend::inverse(MatrixComplex a) const {
    return scalapack_.inverse(std::move(a));
}
...
MatrixComplex CosmaPxgemmBackend::gemm(const MatrixComplex& a,
    ...
    auto dist_a = matrix::scatter_replicated_to_distributed(op_a, grid);
    auto dist_b = matrix::scatter_replicated_to_distributed(op_b, grid);
    auto dist_c = matrix::distributed_gemm(dist_a,
                                           dist_b,
                                           MatrixTranspose::NoTranspose,
                                           MatrixTranspose::NoTranspose,
