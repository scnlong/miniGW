...
public:
    ...
    void allocate(std::size_t count) {
        ...
        count_ = count;
        if (count_ > 0) {
            check_hip(hipMalloc(reinterpret_cast<void**>(&ptr_), count_ * sizeof(T)), "hipMalloc(DeviceBuffer)");
        }
    }
...
public:
    ...
    void allocate(std::size_t count) {
        ...
        count_ = count;
        if (count_ > 0) {
            check_hip(hipHostMalloc(reinterpret_cast<void**>(&ptr_), count_ * sizeof(T), hipHostMallocDefault),
                       "hipHostMalloc(PinnedHostBuffer)");
        }
    }
...
__global__ void build_pq_panel_kernel(const double* __restrict__ eri,
    ...
    panel[static_cast<long long>(ph) + static_cast<long long>(kk) * nph] = make_gpu_complex(value, 0.0);
}

[[nodiscard]] HipPqPhPanelStorageMode choose_mode(HipPqPhPanelStorageMode requested,
                                                     std::size_t eri_bytes,
                                                     std::size_t panel_bytes) {
    if (requested != HipPqPhPanelStorageMode::Auto) {
        return requested;
    }
    ...
    // Keep this deliberately conservative because HipScreeningWorkspace also
    // holds several n_ph^2 complex matrices and hipSOLVER workspace.  Full ERI
    ...
    return (eri_bytes + panel_bytes < budget)
               ? HipPqPhPanelStorageMode::FullEriResident
               : HipPqPhPanelStorageMode::StreamingPanel;
}

} // namespace

std::string_view to_string(HipPqPhPanelStorageMode mode) noexcept {
    switch (mode) {
        case HipPqPhPanelStorageMode::Auto: return "auto";
        case HipPqPhPanelStorageMode::FullEriResident: return "full-eri-resident";
        case HipPqPhPanelStorageMode::StreamingPanel: return "streaming-panel";
    }
    return "unknown";
}

struct HipPqPhPanelView::Impl {
    Impl(const MolecularIntegrals& integrals,
         const ParticleHoleBasis& ph_basis,
         std::size_t max_panel_width_in,
         HipPqPhPanelStorageMode requested_mode)
        ...
        if (nmo <= 0 || nph <= 0) {
            throw std::runtime_error("HipPqPhPanelView: empty ERI/ph basis is not supported");
        }
        ...
        if (eri.dim0() != static_cast<std::size_t>(nmo) ||
            ...
            throw std::runtime_error("HipPqPhPanelView: ERI tensor shape mismatch");
        }
        ...
        check_hip(hipMemcpy(d_occ.get(), h_occ.data(), h_occ.size() * sizeof(int), hipMemcpyHostToDevice),
                   "hipMemcpy(ph occ H2D)");
        check_hip(hipMemcpy(d_virt.get(), h_virt.data(), h_virt.size() * sizeof(int), hipMemcpyHostToDevice),
                   "hipMemcpy(ph virt H2D)");
        ...
        if (mode == HipPqPhPanelStorageMode::FullEriResident) {
            ...
            check_hip(hipMemcpy(d_eri.get(), eri.data().data(), eri.data().size() * sizeof(double), hipMemcpyHostToDevice),
                       "hipMemcpy(ERI H2D)");
        } else {
    ...
    void ensure_panel_capacity(std::size_t width_in) {
        ...
        if (mode == HipPqPhPanelStorageMode::StreamingPanel) {
    ...
    void fill_panel_streaming(std::size_t p_index, std::size_t k_begin, std::size_t width) {
        ...
        for (std::size_t kk = 0; kk < width; ++kk) {
            ...
            for (std::size_t ph = 0; ph < static_cast<std::size_t>(nph); ++ph) {
                ...
                panel[ph + kk * static_cast<std::size_t>(nph)] = make_gpu_complex(value, 0.0);
            }
        }
    ...
    HipComplexPanelView fill_panel(std::size_t p_index, std::size_t k_begin, std::size_t width_in) {
        if (p_index >= static_cast<std::size_t>(nmo)) {
            throw std::runtime_error("HipPqPhPanelView::fill_panel: p index out of range");
        }
        if (k_begin > static_cast<std::size_t>(nmo) || width_in > static_cast<std::size_t>(nmo) - k_begin) {
            throw std::runtime_error("HipPqPhPanelView::fill_panel: k panel exceeds nmo");
        }
        if (width_in == 0) {
            last_panel_width = 0;
            return HipComplexPanelView{d_panel.get(), static_cast<std::size_t>(nph), 0};
        }
        ...
        if (mode == HipPqPhPanelStorageMode::FullEriResident) {
            ...
            build_pq_panel_kernel<<<blocks, threads>>>(d_eri.get(), d_occ.get(), d_virt.get(), d_panel.get(),
                                                       ...
                                                       width);
            check_hip(hipGetLastError(), "build_pq_panel_kernel");
        } else {
            fill_panel_streaming(p_index, k_begin, width_in);
        }

        last_panel_width = width_in;
        return HipComplexPanelView{d_panel.get(), static_cast<std::size_t>(nph), width_in};
    }
    ...
    HipPqPhPanelStorageMode mode{HipPqPhPanelStorageMode::StreamingPanel};
...
HipPqPhPanelView::HipPqPhPanelView(const MolecularIntegrals& integrals,
                                         ...
                                         HipPqPhPanelStorageMode mode)
    : impl_(std::make_unique<Impl>(integrals, ph_basis, max_panel_width, mode)) {}

HipPqPhPanelView::~HipPqPhPanelView() = default;
HipPqPhPanelView::HipPqPhPanelView(HipPqPhPanelView&&) noexcept = default;
HipPqPhPanelView& HipPqPhPanelView::operator=(HipPqPhPanelView&&) noexcept = default;

std::size_t HipPqPhPanelView::nmo() const noexcept {
    return impl_ ? static_cast<std::size_t>(impl_->nmo) : 0;
}

std::size_t HipPqPhPanelView::nph() const noexcept {
    return impl_ ? static_cast<std::size_t>(impl_->nph) : 0;
}

std::size_t HipPqPhPanelView::max_panel_width() const noexcept {
    return impl_ ? static_cast<std::size_t>(impl_->max_panel_width) : 0;
}

std::size_t HipPqPhPanelView::last_panel_width() const noexcept {
    return impl_ ? impl_->last_panel_width : 0;
}

std::size_t HipPqPhPanelView::estimated_device_bytes() const noexcept {
    return impl_ ? impl_->estimated_device_bytes() : 0;
}

std::size_t HipPqPhPanelView::estimated_host_pinned_bytes() const noexcept {
    return impl_ ? impl_->estimated_host_pinned_bytes() : 0;
}

HipPqPhPanelStorageMode HipPqPhPanelView::storage_mode() const noexcept {
    return impl_ ? impl_->mode : HipPqPhPanelStorageMode::StreamingPanel;
}

std::string_view HipPqPhPanelView::storage_mode_name() const noexcept {
    return to_string(storage_mode());
}

HipComplexPanelView HipPqPhPanelView::fill_panel(std::size_t p_index,
                                                       std::size_t k_begin,
                                                       std::size_t width) {
    if (!impl_) {
        throw std::runtime_error("HipPqPhPanelView::fill_panel called on moved-from object");
    }
