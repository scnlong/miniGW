...
long double MemoryFootprint::persistent_runtime_bytes() const {
    return input_bytes()
         + runtime_copy_bytes()
         + sigma_x_bytes
         + v_ph_matrix_bytes
...
long double MemoryFootprint::per_frequency_workspace_bytes() const {
...
long double MemoryFootprint::peak_bytes() const {
    return persistent_runtime_bytes() + per_frequency_workspace_bytes();
}
...
MemoryFootprint estimate_memory_footprint(const GwInput& input,
    ...
    footprint.n_ph = n_ph;
    footprint.num_freq_points = num_freq_points;
    ...
    footprint.frequency_grid_bytes = bytes_for({num_freq_points}, sizeof(double))
                                   + bytes_for({num_freq_points}, sizeof(double))
                                   + bytes_for({num_freq_points}, sizeof(Complex));
...
void print_memory_footprint_report(const MemoryFootprint& footprint) {
    ...
    std::cout << "  Input arrays resident in GwInput:\n";
    ...
    std::cout << "  Current runtime resident copies:\n";
    ...
    std::cout << "  Main GW resident arrays:\n";
    ...
    std::cout << "  Per-frequency peak workspace:\n";
    ...
    std::cout << "  Totals:\n";
    std::cout << "    Persistent subtotal:  " << format_bytes(footprint.persistent_runtime_bytes()) << '\n';
    std::cout << "    Workspace subtotal:   " << format_bytes(footprint.per_frequency_workspace_bytes()) << '\n';
    std::cout << "    Estimated peak:       " << format_bytes(footprint.peak_bytes()) << '\n';
    std::cout << "  Note: this is an algorithmic host-side estimate. It excludes allocator overhead,\n"
              << "        LAPACK/ScaLAPACK/cuSolver, etc internal workspaces, device memory, and \n"
			  << "        OS/runtime overhead.\n\n";
}
